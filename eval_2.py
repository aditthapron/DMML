...
def main(gid=None, dataset=None, dataset_root=None, which=None, exp_dir=None, verbose=False):
    ...
    WHICH = '1165'                     # which model to load
    EXP_DIR = './exp/dmml_1/VRIC'
    ...
    for i in range(len(query_label)):
        ap_tmp, CMC_tmp,index = evaluate(query_feature[i], query_label[i], query_cam[i],
    ...
    np.savez('out_2.npz',ind=np.array(index_all),gallery=gallery_label,query=query_label)
